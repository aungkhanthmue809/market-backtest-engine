...
def load_data(start_date , end_date):
    ...
    _5m = glob.glob("data_shelf/5m/*.csv")
    _1h = glob.glob("data_shelf/1h/*.csv")
    ...
    df_1h["open_time"] = df_1h["open_time"].astype(str).str[:13]
    df_1h["close_time"] = df_1h["close_time"].astype(str).str[:13]
    df_5m["open_time"] = df_5m["open_time"].astype(str).str[:13]    
    df_5m["close_time"] = df_5m["close_time"].astype(str).str[:13]
